...
Use the `invoke_subagent` tool to launch the `code-testing-builder` sub-agent to compile. Build only the specific test project, not the full solution.

If build fails: use the `invoke_subagent` tool to launch `code-testing-fixer`, rebuild, retry up to 3 times.

### 6. Verify with Tests

Use the `invoke_subagent` tool to launch the `code-testing-tester` sub-agent to run tests.
...
If a lint command is available, use the `invoke_subagent` tool to launch the `code-testing-linter` sub-agent.
