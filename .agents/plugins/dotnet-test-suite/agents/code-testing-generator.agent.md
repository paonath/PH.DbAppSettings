...
Use the `invoke_subagent` tool to launch the `code-testing-researcher` subagent with this task:

```text
- TypeName: code-testing-researcher
...
Use the `invoke_subagent` tool to launch the `code-testing-planner` subagent with this task:

```text
- TypeName: code-testing-planner
- Role: Test Planner
...
Execute each phase by using the `invoke_subagent` tool to launch the `code-testing-implementer` subagent — once per phase, sequentially. For each phase, use:

```text
- TypeName: code-testing-implementer
