...
See [`references/framework-fixes.md`](references/framework-fixes.md) for detection patterns and edit targets per styling method (Pure CSS, SCSS, Tailwind, styled-components, CSS Modules, etc.).
...
1. **Selector-based Search**
   - Search codebase by class name or ID using TokenSave MCP tools (specifically `tokensave_search` or `tokensave_context`) for fast results.
   - Explore style definitions with TokenSave, falling back to `grep_search` only if needed.

2. **Component-based Search**
   - Identify components from element text or structure.
   - Explore related component files with TokenSave tools (specifically `tokensave_context` or `tokensave_search`).
...
- Reload browser (or wait for HMR) and capture screenshots of fixed areas.
- Verify fixes haven't broken other areas; confirm responsive display is intact.
- If issues remain, return to Step 2. If more than 3 attempts for a specific issue, consult the user.
...
- ✅ Use `headroom_compress` to compress large DOM structures, HTML snapshots, or console logs before reasoning over them.
