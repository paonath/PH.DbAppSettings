...
4.  **CODEBASE SEARCH**: Use TokenSave MCP tools (specifically `tokensave_search` or `tokensave_files`) to locate target project files and verify package references before execution.
5.  **RESTORE OUTPUT COMPRESSION**: If `dotnet restore` or package search results generate verbose console output (>100 lines), use `headroom_compress` to compress the output text before reasoning over it.
