...
public sealed class IngestCommandSettings : CommandSettings
{
    [Description("Path to the appsettings.json file to ingest and delete.")]
    ...
    [Description("Skip confirmation prompt and delete the source file immediately.")]
    [CommandOption("-y|--yes")]
    public bool Yes { get; set; }
}

public sealed class IngestCommand : AsyncCommand<IngestCommandSettings>
{
    ...
    public async Task<int> ExecuteAsync(IngestCommandSettings settings)
    {
        ...
        // Confirm and delete source file
        var shouldDelete = settings.Yes;
        if (!shouldDelete)
        {
            shouldDelete = AnsiConsole.Confirm($"Do you want to securely delete the source file [yellow]'{settings.FilePath}'[/]?", defaultValue: false);
        }

        if (shouldDelete)
        {
            File.Delete(settings.FilePath);
            AnsiConsole.MarkupLine($"[bold red]Source file deleted:[/] {settings.FilePath}");
        }
        else
        {
            AnsiConsole.MarkupLine($"[grey]Source file preserved:[/] {settings.FilePath}");
        }
